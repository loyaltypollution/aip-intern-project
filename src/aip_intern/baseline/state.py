...
class BaselineState(TypedDict):
    ...
    triage_result_text: NotRequired[str]   # fallback text if file write tool is skipped
    brief_result_text: NotRequired[str]    # fallback text if file write tool is skipped
    response_result_text: NotRequired[str] # fallback text if file write tool is skipped
    ...
    node_metrics: list[dict[str, Any]]  # per-node metrics for aggregation into RunMetrics
